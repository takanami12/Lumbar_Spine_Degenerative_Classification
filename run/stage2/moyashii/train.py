    ...
    with tqdm(train_dataloader, leave=True) as pbar:
        ...
        for idx, batch in enumerate(pbar):
            ...
            if torch.all(labels == -100):
                ...
                # 分割画像を用意できなかったstudyはtargetに-100を設定することでloss計算から除外する。
...
def evaluate_pretrained(
    ...
    if "state_dict" in state_dict:  # case: wrapped checkpoint
    ...
    with torch.no_grad():
        for batch in tqdm(valid_dataloader, desc=f"Fold {fold} eval"):
            sagittal_t1_image, sagittal_t2_image, axial_t2_image, labels, slice_labels, slice_weights, \
                level_labels, study_ids, level_ids, condition_ids = batch
            if torch.all(labels == -100):
                continue
            ...
            outputs = model(sagittal_t1_image, sagittal_t2_image, axial_t2_image,
                            labels, slice_labels.to(device), slice_weights.to(device), level_labels.to(device),
                            force_loss_execute=False)
...
def main_eval(args: argparse.Namespace):
    settings = load_settings()
    config = get_config(args.config, args.options)
    device = torch.device(config['device'])

    label_df = pd.read_csv(settings.train_data_clean_dir / config['dataset']['label_csv_path']).fillna(-100)
    label2id = {'Normal/Mild': 0, 'Moderate': 1, 'Severe': 2}
    label_df = label_df.replace(label2id)
    image_df = pd.read_csv(settings.train_data_clean_dir / config['dataset']['image_csv_path'])
    coord_df = pd.read_csv(settings.raw_data_dir / config['dataset']['coord_csv_path'])

    # metrics + submit
    metrics_train_df = pd.read_csv(settings.raw_data_dir / config['metrics'].pop('train_csv_path'))
    metrics = RSNA2024Metrics(metrics_train_df, **config['metrics'])
    submit = Submit(**config['submit'])
    ...
    for fold in config['folds']:
        print(f"========== Fold {fold} ==========")
        fold_valid_df = label_df[label_df['fold'] == fold].reset_index(drop=True)
        ...
        ckpt_path = Path("/kaggle/input/rsna2024-lsdc-3rd-models-pub/stage2/tkmn/side/v9/0024") / f"exp0024_fold{fold}_best_score.pth"

        # evaluate
        results = evaluate_pretrained(model_class, config, fold, valid_dataloader, device, metrics, submit, ckpt_path)

        print("Fold Results:")
        print(f"Accuracy: {results['accuracy']:.4f}")
        print(f"F1-macro: {results['f1_macro']:.4f}")
        print(f"F1-weighted: {results['f1_weighted']:.4f}")
        print("F1 per class:", results["f1_per_class"])
...
if __name__ == "__main__":
    ...
    main_eval(args)   # <-- call evaluation instead of training
