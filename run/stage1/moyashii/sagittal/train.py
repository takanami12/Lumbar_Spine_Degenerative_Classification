...
def validation_one_epoch(
    ...
    metrics_results, _, _, _ = metrics(keypoints, preds)
...
def main(args: argparse.Namespace):
    ...
    for fold in config['folds']:
        ...
        # data loader
        dataset_class = getattr(datasets, config['dataset'].pop('name'))
        ...
        model = RSNA2024KeypointNet(**config['model'])
        if 'pretrained' in config:
            model.load_state_dict(torch.load(config['pretrained'], map_location="cpu"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dst_root', type=str, default='sagittal_kpt',
